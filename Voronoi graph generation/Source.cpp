...
void read(int & number, vector<site>& site_point, int & test_point_num, vector<point>& test_point) {
	ifstream in;
	in.open("data.txt");
	in >> number;
	int s = 0;
...
int main() {
	...
	int count = 1;
	for (auto q : test_point) {
		face* test_face = face_vec.at(face_vec.size() / 2);
		int count = 0;
		bool stop = true;
		...
		cout << site_point[test_face->id].id << endl;
	}
	return 0;
}
